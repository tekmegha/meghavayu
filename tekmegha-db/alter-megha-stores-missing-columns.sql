-- ============================================
-- ALTER Commands for Missing Columns in megha_stores Table
-- Execute these commands to add missing columns for invoice functionality
-- ============================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'megha_stores_invoice_template_id_fkey'
        AND table_name = 'megha_stores'
...
-- Verification Query - Check if all columns were added successfully
SELECT 
    column_name, 
    data_type, 
    is_nullable, 
...
AND column_name IN (
    ...
    'tax_id', 'pan_number', 'business_registration_number',
    'features', 'social_links', 'settings',
    'invoice_template_id', 'enable_products', 'enable_cart', 'enable_payments',
    'enable_inventory', 'enable_invoices', 'enable_customers', 'enable_reports',
    'is_verified', 'is_prod_ready', 'deleted_at'
)
...
AND kcu.column_name = 'invoice_template_id';
...
AND indexname LIKE '%city%' OR indexname LIKE '%state%' OR indexname LIKE '%tax_id%' 
OR indexname LIKE '%verified%' OR indexname LIKE '%template%' OR indexname LIKE '%invoices%' 
OR indexname LIKE '%prod_ready%'
ORDER BY indexname;
