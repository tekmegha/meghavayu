...
@Component({
  selector: 'app-home-megha',
  imports: [CommonModule, RouterLink, SkeletonLoaderComponent, ProductTileComponent, StoreSelectorComponent],
  templateUrl: './home-megha.html',
  styleUrl: './home-megha.scss'
})
export class HomeMegha implements OnInit, OnDestroy {
  isLoading = true;
  products: Product[] = [];
  isOnline = true;
  ...
  private async loadProductsForStore(store: StoreSession) {
    try {
      ...
      this.products = data || [];
      this.coffeeCategories = this.getCategoriesForStore(store);
    } catch (error) {
      console.error('Error loading products:', error);
    } finally {
  ...
  private getCategoriesForStore(store: StoreSession): { name: string; icon: string; route: string; slug: string }[] {
    // Megha-specific categories
    return [
      { name: 'Store Management', icon: 'store', route: '/menu?category=stores', slug: 'stores' },
      { name: 'Inventory', icon: 'inventory', route: '/menu?category=inventory', slug: 'inventory' },
      { name: 'Billing', icon: 'receipt', route: '/menu?category=billing', slug: 'billing' },
      { name: 'Reports', icon: 'analytics', route: '/menu?category=reports', slug: 'reports' }
    ];
  }

  private initializeBrandForStore(store: StoreSession) {
    console.log('HomeMegha component - Store selected:', { storeCode: store.storeCode, storeName: store.storeName });
    ...
    if (currentPath === '/home') {
      ...
      console.log('HomeMegha component - Staying on global home route for store selector');
      return;
    }
